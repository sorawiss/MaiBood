...
router.get('/api/get-inpost/:id', async (req, res) => {
    ...
    try {
        ...
        const [result] = await connection.query(
            'SELECT f.id, f.owner, f.material, f.exp, f.is_store, f.image, f.price, f.type, m.fname, m.lname, m.address FROM fridge f INNER JOIN members m ON f.owner = m.id WHERE f.is_store = true AND f.id = ?', [id]);
